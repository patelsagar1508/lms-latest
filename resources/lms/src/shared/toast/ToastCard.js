...
const ToastCard = (props) => {
    ...
    const iconColor = type === toastType.ERROR
        ?  'toast-card__icon--error' : type === toastType.WARNING
        ? 'toast-card__icon--warning' : 'toast-card__icon--success';

    const renderCard = () => {
        return (
            <div className="toast-card__alert">
                <div className={`toast-card__icon ${iconColor}`}>
                <i className={`fas ${type === toastType.ERROR ? 'fa-times-circle' : type === toastType.WARNING ? 'fa-exclamation' : 'fa-check-circle'} `}/>
                </div>
                <div>
                    <h5>
                        {getFormattedMessage(`${type === toastType.ERROR
                            ? 'toast.error.message' : type === toastType.WARNING
                            ? 'toast.try.message' : 'toast.success.message'}`)}
                    </h5>
                    <span>{text}</span>
                </div>
            </div>
