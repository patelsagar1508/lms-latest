...
const ReactTable = (props) => {
    const {
        defaultLimit = Filters.OBJ.limit, isShortEmptyState, paginationComponentOptions = { rowsPerPageText: 'Records per page'},
        ...
        emptyStateMessageId = '', filterKeyName = 'filterItem', emptyNotFoundStateMessageId = '', icon, isTypeFilter
    } = props;
    ...
    const renderRowPerPage = () => {
        setTimeout(() => {
            ...
            if (spans.length > 0)
                spans[0].innerHTML = intl.formatMessage({ id: 'react-data-table.record-per-page' });
        }, 300);
    };
    ...
    const onChangeDidMount = (page) => {
        const filters = {
            ...
            searchText === '' ? filterText === 1 || filterText === 2 || filterText === undefined ? '' : filterText.toLowerCase() : '' ||
            ...
            payment_type: isTypeFilter ? filterText : '',
