...
export const fetchAllBooks = (param) => async (dispatch) => {
    dispatch(setLoading(true));
    await apiConfig.get(apiBaseURL.SEARCH_BOOK_ALL+'?limit=20')
        .then((response) => {
            dispatch({ type: bookActionType.FETCH_DEFAULT_BOOKS, payload: response.data.data });
            dispatch(setLoading(false));
        })
        .catch(({ response }) => {
            dispatch(addToast({ text: response.data.message, type: toastType.ERROR }));
...
export const fetchHomeBooks = (param) => async (dispatch) => {
    dispatch(setLoading(true));
    await apiConfig.get(apiBaseURL.SEARCH_BOOK_ALL+'?limit=4')
        .then((response) => {
            dispatch({ type: bookActionType.FETCH_HOME_SETTINGS, payload: response.data.data });
            dispatch(setLoading(false));
        })
        .catch(({ response }) => {
            dispatch(addToast({ text: response.data.message, type: toastType.ERROR }));
    ...
    await axios.get(environment.URL + '/api/' + apiBaseURL.BOOK + param)
        .then((response) => {
            ...
            if (response.data.totalRecords === 0) {
                dispatch(addToast({ text: 'Sorry!! Books not found.', type: toastType.WARNING }));
            }
