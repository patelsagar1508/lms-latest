...
import {findBooks,resetSearchBooks, resetAllSearchBooks} from '../../store/actions/bookSearchAction';
import {fetchAllBooks} from '../../store/actions/bookAction';
import {fetchSettings} from "../../store/actions/getLanguageAction";
import {fetchAuthors} from '../../store/actions/authorAction';


const BookSearch = (props) => {
    const {isLoading, books, searchBooks, authors, fetchBooks, findBooks, fetchAuthors, location, fetchSettingsdefaultbookReducer, fetchAllBooks, defaultbookReducer, resetSearchBooks, resetAllSearchBooks} = props;
            ...
            <Row>
                ...
                <Col sm={12}>
                    <div className="sticky-table-container">
                        <Card>
                            <CardBody className="book-main-section">
                                <Row>
                                    <Col xs={12}
                                    ...
                                    <Col xs={12} className="mt-3">
                                        ...
                                        {defaultbookReducer.length > 0 && searchBooks.length <= 0 && !isSearch ? <BookSearchTable books={defaultbookReducer}/> : null }
                                    </Col>
                                    <ProgressBar/>
                                </Row>
                            </CardBody>
...
BookSearch.propTypes = {
    ...
    resetAllSearchBooks: PropTypes.func
};

const mapStateToProps = (state) => {
    const {books, searchBooks, authors, isLoading, defaultbookReducer} = state;
    return {books, searchBooks, authors: prepareFullNames(authors), isLoading, defaultbookReducer}
};
export default connect(mapStateToProps, {fetchSettings, fetchBooks, fetchAuthors, findBooks, fetchAllBooks, resetSearchBooks, resetAllSearchBooks})(BookSearch);
