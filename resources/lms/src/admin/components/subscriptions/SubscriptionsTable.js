...
const SubscriptionsTable = (props) => {
    const {subscriptions, onClickModal, setActiveInactive, history, isLoading, totalRecord, onChangeData} = props;
    ...
    const columns = [
        ...
        {
            name: getFormattedMessage("subscription-table.column.subscription-plan.label"),
            selector: 'plan_name',
            sortable: true,
            cell: row => {
        ...
        {
            name: getFormattedMessage("subscription-table.column.start-date.label"),
            selector: 'start_date',
            sortable: true,
        ...
        {
            name: getFormattedMessage("subscription-table.column.end-date.label"),
            selector: 'end_date',
            sortable: true,
        ...
        },
        {
            ...
            selector: 'type',
            sortable: true,
            cell: row => <span>{row.type === 1 ? getFormattedMessage("stripe.label") : getFormattedMessage("Offline_label")}</span>
        ...
        },
        {
            ...
            cell: (row) =>
                <div className="member-form__switch">
                    <Field name="is_active" checked={row.status} component={ToggleSwitch}
    ...
    return (
        <ReactDataTable items={subscriptions} columns={columns} emptyStateMessageId="users.empty-state.title"
                        emptyNotFoundStateMessageId="users.not-found.empty-state.title" isShowFilterField isTypeFilter={true}
...
const subscriptionForm = reduxForm({form: 'subscriptionForm'})(SubscriptionsTable);
