...
const BookItemForm = (props) => {
    const {
        ...
        fetchPublishers, handleSubmit, newBookItem = false, e_book_url = ''
    } = props;
    ...
    return (
        <Row className="animated fadeIn book-form m-3">
            <Col xs={12} sm={6}>
                <Field name="book_code" className="inputBox" label="books.items.input.book-code.label"  inputRef={inputRef} required
                       onChange={(e) => enableDisableUserInput(e, maxDigits.BOOK_CODE)} type="text"
            ...
            </Col>
            {
                formateOptions && formateOptions.id === 3 && e_book_url ?
                    <Col xs={12} sm={6} className="pt-4">
                        View your PDF <i className="fa fa-download fa-md cursor-pointer text-info ml-2" onClick={() => onClickOpenEBook(e_book_url)}/>
                    </Col>:
            ...
            <Col xs={12} sm={6}>
                <Field name="location" label="books.items.input.location.label" groupText="map-pin"
                       component={InputGroup} type="text"/>
            </Col>
            {formateOptions && formateOptions.id !== 3 ?
                <Col xs={12} sm={6}>
