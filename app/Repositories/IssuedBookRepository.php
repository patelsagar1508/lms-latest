...
class IssuedBookRepository extends BaseRepository implements IssuedBookRepositoryInterface
{
    ...
    {
        $member = Member::where('id',$input['member_id'])->with('subscription.subscriptionPlan')->first();

        $issuedBooksCount = IssuedBook::ofMember($input['member_id'])->issued()->count();
        if ($issuedBooksCount >=  $member->subscription->subscriptionPlan->issue_books_limit) {
    ...
    public function reserveBook($input)
    {
        ...
        if ($reserveBooksCount >=  $member->subscription->subscriptionPlan->reserve_books_limit) {
